...
int main()
{
    ...
    while (window.isOpen())
    {
        ...
        else if (y < height2) 
        {
            ...
            {
                y = height2;
                platforms[i]->setY(platforms[i]->getY() - dy);

                if (platforms[i]->getY() > height) 
                {
                    platforms[i]->setX(rand() % width);
                    platforms[i]->setY(0);
                }
            }
        }
        ...
        for (int i = 0; i < platforms.size(); i++) 
        {
            if ((x + 40 > platforms[i]->getX()) 
                && (x + 40 < platforms[i]->getX() + 105)
                && (y + 64 > platforms[i]->getY()) 
                && (y + 64 < platforms[i]->getY() + 50) 
                && (dy > 0))
                {
                    dy = -10;
                }
        ...
        for (int i = 0; i < platforms.size(); i++) 
        {
            greenSpr.setPosition(platforms[i]->getX(), platforms[i]->getY());
